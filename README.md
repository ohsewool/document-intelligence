...
python3 -m pytest tests/ -q     # 224 tests
```
...
```bash
python3 -m pytest tests/ -q      # 224 tests, about 12 seconds
```
...
"모델은 파서가 내놓은 것을 그대로 받아 같은 방식으로 검증한다"는 이 저장소의 중심 주장인데, 파서가 하나뿐이었다. 어댑터가 좌표를 잘못 만들어도 — PDF의 아래쪽 원점을 위쪽으로 뒤집는 것을 빼먹어도 — **그 좌표들끼리는 모순이 없으니 모델은 전부 받아들인다.** 자기 자신과 일관된 것과 문서의 그 자리를 가리키는 것은 다른 주장이다.

`pypdf`를 **두 번째 어댑터가 아니라 검사자로** 붙였다. pdfplumber는 pdfminer 위에 있어서 pdfminer를 직접 쓰면 같은 엔진을 두 번 부르는 셈이고, pypdf는 텍스트 행렬을 직접 읽는 다른 구현이다. 어댑터를 만들지 않은 이유는 분명하다 — pypdf의 visitor는 시작점만 주고 **너비를 주지 않는다.** 글자 수로 추정하면 인용이 글자가 없는 곳을 가리키는데, 그게 이 모델이 막으려는 실패다.

| | 일치율 | 잔차 표준편차 |
|---|---|---|
| 본문 쪽 (10장) | 91.7% ~ 100% | 0.04 ~ 3.91 pt |
| 표·그림 쪽 (5장) | 13.6% ~ 74.8% | 14.69 ~ 50.80 pt |

**첫 판은 이빨이 없었다.** "조각마다 가까운 줄이 있는가"만 물었더니 **좌표를 위아래로 뒤집어도 80.7%가 통과했다** — 줄 간격이 11~14pt인 지면에서는 아무 높이나 6pt 안에 이웃이 있다. 허용치를 좁혀도 갈리지 않았다. 갈리는 것은 근접이 아니라 **일관성**이었다: 방향이 맞으면 두 엔진의 차이는 모든 줄에서 같은 값(디센더)이고, 뒤집히면 무작위가 된다. 지금은 뒤집은 좌표로 먼저 돌려 무너지는 것을 본 뒤에 정상 결과를 신뢰한다.

표·그림 쪽 불일치는 **허용치를 늘려 덮지 않았다.** 덮으면 그림 쪽 인용이 본문 쪽만큼 믿을 만해 보인다. 실제로는 방향이 맞는데도 두 엔진이 글자 위치를 합의하지 못한다 — 그런 지면으로의 인용은 다르게 취급돼야 하고, 아무것도 그렇게 말하고 있지 않았다.

### 거부가 시험된 적이 없었다

`reading_order.py`는 85%가 실행되고 있었고, **실행되지 않는 15%는 전부 거부 분기**였다. 잘못된 입력을 막는 `raise`가 열넷 있는데 그중 아무것도 한 번도 발동한 적이 없었다.

"표현할 수 없는 것은 거부한다"가 이 저장소의 논지다. 거부가 시험된 적이 없으면 그건 코드에 적힌 문장이지 성질이 아니다 — **조건 하나가 뒤집혀 있어도 정상 입력만으로는 아무 차이가 없다.** 전부 발동시켜 봤고 **열넷 다 동작한다.** 결함은 없었고, 다음에 뒤집혔을 때 알아차리게 하는 것이 이 테스트의 일이다.

메시지까지 고정한 이유가 있다. 순수 순환(`a→b→a`)은 "종점이 정확히 하나여야 한다"로 걸린다 — 종점 검사가 먼저 돌기 때문이고 그것도 참인 서술이다. **끊긴 순환**을 가리키는 메시지는 다른 입력에서만 나온다(온전한 사슬에 따로 도는 순환이 붙은 경우). 메시지를 안 보면 두 경로가 같아 보이고, 한쪽이 죽은 코드가 돼도 모른다.

남은 세 줄은 각각 다르게 처리했다. **중복 페이지 키**는 `dict`으로는 만들 수 없지만 서명이 `Mapping`을 받으므로 커스텀 `Mapping`으로 도달 가능하다 — 테스트를 넣었다. **시작점 개수 검사와 순환 검사**는 구역 4개까지 전수 탐색으로 **도달 불가**임을 확인했다(앞선 검사가 모두 선점한다). 지우지 않고 `pragma: no cover`와 이유를 달았다 — **실행되지 않는 검사를 활성 검사처럼 두면 읽는 사람이 이 함수의 보호 범위를 실제보다 넓게 본다.**

### 분기로 다시 재봤고, 여기는 이미 100%였다

한 번도 실행되지 않는 줄 7개를 채워 구문 커버리지 100%를 만든 다음 날, **관문 자체를 의심했다.** 구문 커버리지는 "모든 줄이 실행됐다"만 말하지 각 `if`가 양쪽으로 다 가봤다는 뜻이 아니다.

`--branch`로 다시 쟀다. **244개 분기가 전부 양쪽으로 돌고 있었다** — 형제 넷 중 유일하다(`agent-safety-core` 4개, `mcp-gateway` 5개, `rag-profile-selector` 1개가 남아 있었다). **빈손인 감사도 결과이고, 아무도 돌리지 않은 감사와는 다르다.** 관문도 `--branch`로 올렸다.

### 그 감사는 한 파일에서 멈춰 있었다

위의 확인은 `reading_order.py` 하나에 대한 것이었다. **나머지 세 모듈에는 같은 것이 없었다.**

패키지 전체에 걸어봤다 — 문자열 메시지를 가진 `raise` **76개를 하나씩 `pass`로 바꾸고** 매번 스위트를 돌렸다. 잡힘 50건, **안 잡힘 23건**(그중 둘은 이 저장소가 도달 불가로 선언해둔 줄), 비종료 1건(순환 검사를 지우면 `while`이 영영 돈다 — 그것도 거부가 일한다는 증거다), 여러 줄 `raise`라 변이가 문법을 깨뜨린 것 2건.

**진짜로 안 잡히는 것이 21건이었다.** `coordinates.py`는 13개 중 9개, `hierarchy.py`는 16개 중 9개가 지워도 초록불이었다. `reading_order.py`만 20/20이었고, **그 파일에만 이 감사가 있었기 때문**이다.

그중 하나는 이 저장소가 스스로 경고해둔 함정이었다. `test_rejects_duplicate_regions`는 `assertRaises(ValueError)`로만 확인했는데, 유일성 검사를 통째로 지우면 중복 구역이 그 다음의 "종점이 정확히 하나여야 한다"에 걸린다 — **다른 이유로 거부되고 테스트는 통과한다.** 바로 위 문단이 그 이유를 적어두고 있었고, 그 규칙이 나중에 쓰였을 뿐이다.

**첫 감사는 무효였다.** `--timeout=300`을 넘겼는데 이 저장소에 그 플러그인이 없어 pytest가 사용법 오류로 끝났고, 스크립트는 0 아닌 종료 코드를 전부 "잡힘"으로 읽어 **76/76 초록**을 냈다. 형제 저장소의 변이 도구에서 이미 고친 함정을 새로 만든 하네스가 그대로 반복했다. 기준선 확인과 종료 코드 구분을 넣고 다시 돌린 것이 위 숫자다.

21건 각각을 발동시키는 테스트를 넣었다(`tests/test_rejections_that_were_never_fired.py`). 전부 메시지까지 맞춘다.
