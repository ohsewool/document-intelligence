...
It lives under the package rather than at the top level because a top-level
`adapters` is a name other installed distributions also claim - agent-safety-core
ships one - and whichever is a regular package earliest on the path wins,
silently. A citation library whose parser can be replaced by an unrelated
project's module of the same name is not a small problem.

`adapters/pdfplumber_adapter.py` is kept as a re-export so the previously
documented import keeps working wherever nothing collides.
"""
